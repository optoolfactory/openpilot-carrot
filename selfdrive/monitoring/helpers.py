...
class DRIVER_MONITOR_SETTINGS:
  def __init__(self, device_type):
    ...
    self._PUPILS_NOT_DETECTED_WARNING_TIME = 3.0
...
class DriverMonitoring:
  def __init__(self, rhd_saved=False, settings=None, always_on=False):
    ...
    self.pupils_not_detected_duration = 0.
    self.pupils_not_detected_warning_triggered = False
    self.pupils_not_detected_warning = False
  ...
  def _update_states(self, driver_state, cal_rpy, car_speed, op_engaged, standstill, demo_mode=False, steering_angle_deg=0.):
    ...
    if self.face_detected and self.pose.low_std and not eyes_visible:
      self.pupils_not_detected_duration += self.settings._DT_DMON
      if self.pupils_not_detected_duration >= self.settings._PUPILS_NOT_DETECTED_WARNING_TIME and not self.pupils_not_detected_warning_triggered:
        self.pupils_not_detected_warning = True
        self.pupils_not_detected_warning_triggered = True
    else:
      self.pupils_not_detected_duration = 0.
      self.pupils_not_detected_warning_triggered = False
  ...
  def _update_events(self, driver_engaged, op_engaged, standstill, wrong_gear, car_speed):
    ...
    if self.pupils_not_detected_warning:
      self.current_events.add(EventName.driverDistracted2)
