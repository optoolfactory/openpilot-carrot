...
class DriverMonitoring:
  def __init__(self, rhd_saved=False, settings=None, always_on=False):
    ...
    self.eyes_not_recognized_cnt = 0
  ...
  def _get_distracted_types(self):
    ...
    if self.eyes_not_recognized_cnt >= 60:
      distracted_types.append(DistractedType.DISTRACTED_DROWSY)
  ...
  def _update_states(self, driver_state, cal_rpy, car_speed, op_engaged, standstill, demo_mode=False, steering_angle_deg=0.):
    ...
    if self.face_detected and ((driver_data.eyesVisibleProb <= self.settings._EYE_THRESHOLD) or (self.blink_prob > self.settings._BLINK_THRESHOLD)):
      self.eyes_not_recognized_cnt += 1
    else:
      self.eyes_not_recognized_cnt = 0
    ...
    self.driver_distracted = (DistractedType.DISTRACTED_PHONE in self.distracted_types
                              ...
                              or DistractedType.DISTRACTED_BLINK in self.distracted_types
                              or DistractedType.DISTRACTED_DROWSY in self.distracted_types) \
  ...
  def _update_events(self, driver_engaged, op_engaged, standstill, wrong_gear, car_speed):
    ...
    if DistractedType.DISTRACTED_DROWSY in self.distracted_types:
      self.awareness = min(self.awareness, self.threshold_prompt - 0.01)
    alert = None
    if self.awareness <= 0.:
