...
class CarrotPlanner:
  def __init__(self):
    ...
    self.events = Events()
    self.myDrivingMode = DrivingMode(3) # normal, fixed. DrivingMode(self.params.get_int("MyDrivingMode"))
    ...
    self.cruise_tf_ratio = 1.0
    self.tFollowGap1 = 1.25
    self.tFollowGap2 = 1.45
    self.tFollowGap3 = 1.75
    self.tFollowGap4 = 2.0

    self.dynamicTFollowLC = 0.0
    self.enableSpeedTF = 0
    self.personality = 1

    self.cruise_accel_ratio = 1.0
  ...
  def _params_update(self):
    ...
    if self.params_count % 10 == 0:
      myDrivingMode = DrivingMode(3) # NORMAL fixed. DrivingMode(self.params.get_int("MyDrivingMode"))
      if myDrivingMode != self.myDrivingMode_last:
        self.myDrivingMode_disable_auto = True
      self.myDrivingMode_last = myDrivingMode
      
      self.myDrivingModeAuto = 0 # fixed. self.params.get_int("MyDrivingModeAuto")
    ...
    if self.params_count == 10:
      ...
      self.cruise_tf_ratio = self.params.get_float("CruiseTFRatio") / 100.
      self.dynamicTFollowLC = 0.8 # fixed. self.params.get_float("DynamicTFollowLC") / 100.
      self.enableSpeedTF = self.params.get_int("EnableSpeedTF")
    elif self.params_count == 30:
      self.cruise_accel_ratio = self.params.get_float("CruiseAccelRatio") / 100.
    elif self.params_count == 40:
      self.stop_distance = 6.0 # fixed. self.params.get_float("StopDistanceCarrot") / 100.
      self.eco_over_speed = 0 # disabled, self.params.get_int("CruiseEcoControl")
      self.trafficStopDistanceAdjust = -200 #fixed. self.params.get_float("TrafficStopDistanceAdjust") / 100.
    elif self.params_count >= 100:

      self.params_count = 0

  def get_cruise_accel_ratio(self):
    factor = self.myHighModeFactor if self.myDrivingMode == DrivingMode.High else self.mySafeFactor
    return self.cruise_accel_ratio * factor
  ...
  def get_T_FOLLOW(self, personality=log.LongitudinalPersonality.standard, v_ego=0.0, a_ego=0.0):
    ...
    if self.enableSpeedTF < 0:
      ...
      tf_target *= self.cruise_tf_ratio
  ...
  def update(self, sm, v_cruise_kph):
    ...
    experimental_mode = True if self.xState in [XState.e2ePrepare] else False
    ...
    return v_cruise_kph, experimental_mode
