...
def driver_distracted_1_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  duration = 2.0 if HARDWARE.get_device_type() == 'mici' else 0.1
  if sm.valid['driverMonitoringState'] and (sm['driverMonitoringState'].distractedType & 8):
    return Alert(
      "졸음운전 감지됨",
      "눈동자 미인식",
      AlertStatus.normal, AlertSize.small,
      Priority.LOW, VisualAlert.none, AudibleAlert.none, duration)
  return Alert(
    "Pay Attention",
    "",
    AlertStatus.normal, AlertSize.small,
    Priority.LOW, VisualAlert.none, AudibleAlert.none, duration)
def driver_distracted_2_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  duration = 1.0 if HARDWARE.get_device_type() == 'mici' else 0.1
  if sm.valid['driverMonitoringState'] and (sm['driverMonitoringState'].distractedType & 8):
    return Alert(
      "졸음운전 감지됨",
      "눈동자 미인식",
      AlertStatus.userPrompt, AlertSize.mid,
      Priority.MID, VisualAlert.steerRequired, AudibleAlert.promptDistracted, duration)
  return Alert(
    "Pay Attention",
    "Driver Distracted",
    AlertStatus.userPrompt, AlertSize.mid,
    Priority.MID, VisualAlert.steerRequired, AudibleAlert.promptDistracted, duration)
def driver_distracted_3_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int, personality) -> Alert:
  if sm.valid['driverMonitoringState'] and (sm['driverMonitoringState'].distractedType & 8):
    return Alert(
      "즉시 운전대를 잡으십시오",
      "졸음운전: 눈동자 미인식",
      AlertStatus.critical, AlertSize.full,
      Priority.HIGH, VisualAlert.steerRequired, AudibleAlert.warningImmediate, 0.1)
  return Alert(
...
EVENTS: dict[int, dict[str, Alert | AlertCallbackType]] = {
  ...
  EventName.driverDistracted1: {
    ET.PERMANENT: driver_distracted_1_alert,
  },

  EventName.driverDistracted2: {
    ET.PERMANENT: driver_distracted_2_alert,
  },

  EventName.driverDistracted3: {
    ET.PERMANENT: driver_distracted_3_alert,
  },
